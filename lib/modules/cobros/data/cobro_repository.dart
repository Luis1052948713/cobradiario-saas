...
class CobroRepository {
  ...
  Future<int> _registrarCobroOnline(CobroModel cobro) async {
    ...
    if (prestamoUuid == null || cobradorUuid == null) {
      throw StateError('Prestamo o cobrador online no encontrado.');
    }
    ...
    final estado = prestamo['estado'] as String;
    final saldo = (prestamo['saldo'] as num).toDouble();
    final clienteUuid = prestamo['cliente_id'] as String;
    ...
    final cliente = await SupabaseService.requireClient
        ...
        .eq('id', clienteUuid)
        .single();
    final cobradorAsignadoUuid = cliente['cobrador_id'] as String?;

    if (perfil.esCobrador &&
        (perfil.id != cobradorUuid || perfil.id != cobradorAsignadoUuid)) {
      throw StateError('No puedes cobrar un cliente asignado a otro cobrador.');
    }
    if (estado != AppEstados.activo && estado != AppEstados.atrasado) {
    ...
    await SupabaseService.requireClient.from('auditoria').insert({
      ...
      'descripcion':
          'Cobro prestamo=${cobro.prestamoId} monto=${cobro.monto} saldo_anterior=$saldo saldo_actual=$saldoActual',
      'metadata': {'local_referencia_id': id},
    });
    return id;
  }
